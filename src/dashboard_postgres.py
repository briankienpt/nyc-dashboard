...
with tab1:
    ...
    mc1, mc2, mc3, mc4 = st.columns([2, 1.2, 1, 1])
    with mc1:
        map_metric = st.radio(
    ...
    with mc2:
        map_style_choice = st.selectbox(
            "Giao diện bản đồ:",
            options=["🗺️ Sáng (Carto Positron)", "🌙 Tối (Carto Dark)", "🌐 Đường phố (OSM)"],
            index=0
        )
    with mc3:
        radius_val = st.slider("Bán kính điểm nhiệt (Radius)", 15, 45, 25)
    with mc4:
        zoom_val = st.slider("Độ phóng đại (Zoom)", 8, 13, 10)

    # Map style mapping
    style_map = {
        "🗺️ Sáng (Carto Positron)": "carto-positron",
        "🌙 Tối (Carto Dark)": "carto-darkmatter",
        "🌐 Đường phố (OSM)": "open-street-map"
    }
    selected_style = style_map.get(map_style_choice, "carto-positron")

    if "Giá trung bình" in map_metric:
        target_z = 'med_price'
        color_scale = "Plasma"
        z_title = "Giá trung bình ($)"
    elif "Giá/sqft" in map_metric:
    ...
    # Tính tâm bản đồ linh hoạt theo dữ liệu
    c_lat = float(geo_df['lat'].mean()) if not geo_df.empty else 40.7300
    c_lon = float(geo_df['lon'].mean()) if not geo_df.empty else -73.9350
    ...
    fig_map.update_layout(
        ...
        coloraxis_colorbar=dict(
            ...
            len=0.75,
            thickness=18,
            bgcolor="rgba(255,255,255,0.85)" if selected_style != "carto-darkmatter" else "rgba(30,30,30,0.85)"
        )
    )
    st.plotly_chart(fig_map, width='stretch')

    # Chú giải điểm nóng chi tiết
    top_p_geo = geo_df.sort_values('med_price', ascending=False).head(3)
    top_v_geo = geo_df.sort_values('n_count', ascending=False).head(3)
    top_sqft_geo = geo_df.loc[geo_df['med_ppsf_clean'] > 0].sort_values('med_ppsf_clean', ascending=False).head(3)

    p_spots = ", ".join([f"<b>{r['neighborhood'].title()}</b> ({r['borough_name']}: ${r['med_price']/1e6:.2f}M)" for _, r in top_p_geo.iterrows()])
    v_spots = ", ".join([f"<b>{r['neighborhood'].title()}</b> ({r['borough_name']}: {r['n_count']:,} GD)" for _, r in top_v_geo.iterrows()])
    s_spots = ", ".join([f"<b>{r['neighborhood'].title()}</b> (${r['med_ppsf_clean']:,.0f}/sqft)" for _, r in top_sqft_geo.iterrows()])

    insight_box(f"""
    <b>📍 Phân tích & Nhận diện Điểm nóng (Hotspots) trên Bản đồ:</b><br>
    • 🔴 <b>Điểm nóng Giá bán cao nhất:</b> {p_spots if p_spots else "N/A"}.<br>
    • 🟢 <b>Điểm nóng Thanh khoản sôi động nhất:</b> {v_spots if v_spots else "N/A"}.<br>
    • 📐 <b>Điểm nóng Đơn giá $/sqft cao nhất:</b> {s_spots if s_spots else "N/A"}.<br>
    • 💡 <i>Mẹo tương tác: Sử dụng con lăn chuột để thu phóng (Zoom), thay đổi kiểu Bản đồ (Sáng/Tối) hoặc trượt Bán kính (Radius) để quét chính xác tâm điểm giao dịch của từng khu phố.</i>
    """)
...
with tab3:
    ...
    with st.expander("🔍 Xem thêm: Dữ liệu Toàn cảnh & Phân hóa cấp Quận", expanded=False):
        ...
        st.markdown("<p style='color:#64748b; font-size:14px;'>Đường xu hướng (nét đứt) cho thấy quỹ đạo lợi suất của giá trung bình toàn khu vực đang chọn.</p>", unsafe_allow_html=True)
